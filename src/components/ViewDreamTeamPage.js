    ...
    render() {
        return(
            <div id="dream-team-page">
                {this.props.pageType ? <div id="profile-view">{this.state.existingTeams.length > 0? this.state.existingTeams.map(team => <div className="team-list"><div key={team.id} id={team.id}><h3>{team.team_name}</h3><p>{team.email}</p><button value={team.id} className={team.team_name} onClick={this.getTeamDetails}>Update Team</button><button value={team.id} className={team.team_name} onClick={this.deleteTeam}>Delete Team</button></div></div>) : null}
